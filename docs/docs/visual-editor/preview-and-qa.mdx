...
description: Preview draft A/B test variations with a querystring link, use the debug panel to diagnose issues, and avoid editing live experiments by mistake.
sidebar_label: Preview and QA
...
QA is where good A/B tests are made, or where bad ones get caught before they reach real users. The [Visual Editor](/app/visual) gives you several tools to verify a variation on a real browser, on the real page, with the real SDK, before you publish.

The editor is transparent by design: every change appears in the **Changes panel** (Manual mode) or as **diff cards** you accept one at a time (AI mode). Review them the same way you would a code diff before launch.
...
:::warning Include Drafts exposes all drafts
...
The Visual Editor includes a Debug Panel showing what the SDK evaluated on the active page. Use it to diagnose why a variation isn't showing.

[alyssa insert a screenshot of exactly this feature here: Debug Panel open in the side panel with SDK init, URL targeting, bucketing, and applied visual changes visible.]
...
Experiment-level approval workflows are in development.

:::

Visual experiments can change what production users see. **Approval flows for experiments** (similar to [flag publishing and approval](/features/publishing-and-approval-flows)) will add a review step before an experiment can go live.

Until that ships, use org permissions to limit who can create, edit, and start experiments. Always share preview links with stakeholders before launch.

[alyssa fill out more here: document the approval UX once experiment reviews ship.]
...
[alyssa insert a screenshot of exactly this feature here: amber "live experiment" warning banner at the top of the side panel.]
...
This is sometimes useful (you spotted a typo in a live variation and want to fix it without restarting). It is more often a mistake waiting to happen: you wanted to draft v2 of the variation but accidentally edited v1, and now users are seeing your half-finished work.
