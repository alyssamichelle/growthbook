...
[alyssa fill out more here: add 2–3 internal support tickets or Slack themes that aren't covered yet.]

If your issue isn't here, the **Debug Panel** in the Visual Editor is the single best diagnostic tool. Open it before filing a support ticket. See [Preview and QA](/app/visual/preview-and-qa#the-debug-panel).
...
[alyssa fill out more here: confirm the modifier-key cycle behavior (Alt/Shift) and whether Interactive Mode still exists in the new editor. Update or remove the bullets below once verified.]

- For elements stacked on top of each other (z-index), hold a modifier key (Alt on macOS, Shift on Windows) while clicking to cycle through elements at the same point.
- For open dropdowns or hover menus, use the **Interactive Mode** toggle to keep them open while you select.

## Custom JS changes look "stuck" until refresh

Custom JavaScript injected for a variation does not automatically undo when you select a different element in the editor. Refresh the tab to clear JS side effects and preview from a clean state.

DOM mutation changes (text, styles, classes) revert normally when you reject or revert them in the Changes panel.
...
- **An AI provider key configured** by an org admin in **Settings → General → AI Settings**. See [AI Integrations](/integrations/ai).
