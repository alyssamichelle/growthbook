...
Manual mode is the point-and-click editor inside the [Visual Editor side panel](/app/visual). Click an element on your page, and you get structured controls for content, typography, background and border, spacing, and visibility. You also get escape hatches for arbitrary CSS and JavaScript.

This is the **precision layer** of the Visual Editor. Use it when you know exactly what to change, when you are fine-tuning an [AI mode](/app/visual/ai-mode) draft, or when the change is too small to bother prompting ("set padding to 12px").

Every change you make is listed in the **Changes panel** at the bottom of the side panel. You can review, revert, or edit individual changes before saving the variation. Nothing is hidden.

[alyssa record a video for this section: pick an element, change copy and a style, accept into the Changes panel.]
...
[alyssa insert a screenshot of exactly this feature here: sticky selection bar with tag, classes, picker, tree navigation, and deselect controls.]
...
The **Changes panel** lists every edit in the current variation: text swaps, style updates, image replacements, global CSS, and custom JS. Each row shows what changed and on which selector.

- **Revert** a single change without undoing the whole variation.
- **Switch variations** with the variation dropdown above the panel to confirm you are editing the right arm of the test.

[alyssa insert a screenshot of exactly this feature here: Changes panel with multiple change types listed and the variation switcher visible.]
...
Custom JS requires extra CSP changes on your site. See [Content Security Policy](/app/visual/running-on-your-site#content-security-policy-csp) on the SDK setup page.

:::caution JavaScript does not un-run

Custom JavaScript stays active for the current page session. Effects can remain visible when you move between elements until you refresh the tab. Prefer DOM mutations for declarative edits when possible.
