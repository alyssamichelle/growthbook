...
1. **Upload** — drop in a file from your machine. GrowthBook hosts it on a CDN automatically.
2. **AI generation** — describe the image and let a configured AI model produce it, optionally using the **current image as visual reference**.
3. **Library** — pick from images your organization has already uploaded or generated, reusable across experiments and projects.

When you upload or accept a generated image, GrowthBook stores it in org-scoped storage and serves it from a CDN-backed URL. You do not need to host variation assets yourself.

:::note Image models and pixel dimensions

AI image models choose aspect ratio and rough size from the original image. You cannot pass exact pixel dimensions (e.g. "generate at 1200×630") in the prompt. Resize in [Manual mode](/app/visual/manual-mode) after accepting if you need precise dimensions.

:::

All three paths land in the same `<img>` (or `<picture>`) replacement and are managed identically in the Changes panel.

[alyssa record a video for this section: replace a hero image via Upload, then via AI, then pick the same asset from Library.]
...
[alyssa insert a screenshot of exactly this feature here: Image property group with Upload / AI / Library tabs on a selected `<img>`.]
...
The model used for AI generation is set by your org admin in **Settings → General → AI Settings** under **Visual editor image model**. Self-hosted orgs can choose the model; Cloud uses GrowthBook-managed defaults. See [AI model and brand settings](/app/visual/ai-mode#ai-model-and-brand-settings).
